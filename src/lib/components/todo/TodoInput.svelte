<script>
    import {deleteTodo, toggleComplete, editTodo} from "$lib/store/TodoStore.js";
...
<div class="flex items-center justify-between rounded-md border-2 border-gray-one px-5 py-4">
    <div class="flex w-full max-w-full items-center justify-start">
        ...
        <input
            ...
            class="h-4 w-4 rounded border border-gray-three bg-cream-four text-green-four focus:border-green-five
                    focus:outline focus:outline-2 focus:outline-offset-2 focus:outline-green-five"
        />
        ...
        <input id="{`${todo.id}-text`}"
               ...
               class="ml-5 flex-1 text-ellipsis rounded-none border-x-0 border-t-0 border-b border-dashed
                        border-b-gray-two bg-cream-four px-0 pb-1 text-base font-normal text-gray-three
        ...
        <span class="{completeClass} ml-5 hidden rounded-full py-0.5 px-2 text-sm font-normal
                    text-gray-five md:block">{todo.complete ? 'Complete' : 'In Progress'}</span>
    </div>
    <button type="button" on:click={() => deleteTodo(todo.id)}
            class="group ml-4 flex items-center justify-center bg-cream-four p-2 hover:bg-steel-one
